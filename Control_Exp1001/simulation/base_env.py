...
from Control_Exp1001.utils.utils import set_random_seed
...
class BaseEnv():
    ...
    def __init__(self, dt=1, penalty_calculator=None,
        ...
        if penalty_calculator is None:
            penalty_calculator = DemoPenalty(
                weight_matrix=np.zeros(self.size_yudc[0]),
                S=np.zeros(self.size_yudc[1])
            )
    ...
    def observation_split(self):
        return self.y_star, self.y, self.u, self.c
    ...
    @property
    def y_size(self):
        return self.size_yudc[0]

    @property
    def u_size(self):
        return self.size_yudc[1]

    @property
    def d_size(self):
        return self.size_yudc[2]

    @property
    def c_size(self):
        return self.size_yudc[3]
