...
class ControlBase:
    def __init__(self,
        ...
        self.y_size, self.u_size, self.c_size = y_size, u_size, c_size

        def split(array, lens):
            start = 0
            res = []
            for l in lens:
                res.append(array[start:start + l])
                start += l
            return tuple(res)

        self.indice_y_star, self.indice_y, self.indice_u, self.indice_c = split(
            list(range(y_size * 2 + y_size + u_size + c_size)), [y_size, y_size, u_size, c_size]
        )
        self.log = {}
        self.buffer = []
