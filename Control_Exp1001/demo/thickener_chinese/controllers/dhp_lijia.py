...
class DHP(HDP):
    ...
    def update_model(self,state, action, penalty, next_state, done):
        ...
        while True:
            ...
            # endregion 结束控制量惩罚图
            #####################################
            penalty_all_loss = penalty_all.mean()
            penalty_all_loss.backward(retain_graph=True)

            #####################################
            # region 生成控制量J(k+1)的计算图
            x_pred = self.model_nn(torch.cat((xk, action, c), dim=1))
            x_pred_costate = torch.FloatTensor(self.critic_nn(torch.cat((x_pred, y_star, nc), dim=1)))
            # endregion 结束生成J(k+1)计算图
            #####################################

            x_pred.backward(self.gamma*x_pred_costate) #求J(k+1)对xk的梯度
        ...
        while True:
            ...
            y_pred_costate = y_pred_costate.detach()
            #penalty_u = torch.zeros(J_pred.shape)
            J_loss = penalty_u.mean() + self.gamma * (y_pred.mul(y_pred_costate).mean()*2)
            J_loss_1 = self.gamma * y_pred
            #J_loss = penalty_u.sum()
            ...
            # penalty_u.mean().backward(retain_graph=True)
            # y_pred.backward(y_pred_costate*self.gamma)
            J_loss.backward()
    ...
    def plt_contourf(self, title, X, Y, costate,order):
    ...
    def test_critic_nn(self,title):
        ...
        fig.add_subplot(121)
        self.plt_contourf(title=title+'costate of Height',X=X, Y=Y, costate=costate[0], order=0)

        fig.add_subplot(122)
        self.plt_contourf(title=title+'costate of Concentration',X=X, Y=Y, costate=costate[1], order=1)
        plt.show()
        return
