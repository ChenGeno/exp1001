...
class OneRoundEvaluation():
    def __init__(self, res_list
        ...
        self.y0_grad, self.y1_grad, other_info = zip(*res_list)
    ...
    def evaluate(self):
        ...
        for item in self.res_list:
            other_info = item[-1]
            print(other_info['exp_name'] + ' - ', 'all time', ' - ', float(other_info['time_used'][0]))
            print(other_info['exp_name'] + ' - ', 'train time', ' - ', float(other_info['time_used'][1]))
            print(other_info['exp_name'] + ' - ', 'act time', ' - ', float(other_info['time_used'][2]))
        return mse_dict
