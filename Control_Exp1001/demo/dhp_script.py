...
from Control_Exp1001.control.adp import DHP
...
if dhp.device != torch.device('cpu'):
    dhp.to(dhp.device)
...
sampling_policy = lambda _: np.random.uniform(dhp.u_bounds[:, 0], dhp.u_bounds[:, 1])
...
dhp.normalizer = EnvNormalizer(u_mean, u_std, y_mean, y_std, c_mean, c_std)
dhp.train_identification_model(train_initial_dataset, val_initial_dataset)
...
res_list = [OneRoundExp(controller=dhp, env=env_dhp, max_step=rounds, exp_name='dhp').run()]
