...
class VAEAKFCombinedLinear(nn.Module):
    ...
    def forward_posterior(self, external_input_seq, observations_seq, memory_state=None):
        ...
        self.external_input_seq_embed = self.process_u(external_input_seq)
        self.observations_seq_embed = self.process_x(observations_seq)
        ...
        all_seq = torch.cat([self.external_input_seq_embed, self.observations_seq_embed], dim=2)
        ...
        _, weight_hidden_state = self.dynamic.linears_weight_lstm(
            self.sampled_state, weight_hidden_state
        )
        new_weight_initial_hidden_state = weight_hidden_state
        # endregion

        return self.state_mu, self.state_logsigma, \
               {'posterior_lstm_state': new_posterior_lstm_state,
                'weight_hidden_state': new_weight_initial_hidden_state}
    ...
    def forward_prediction(self, external_input_seq, max_prob=False, memory_state=None):
        ...
        for i in range(l):
            ...
            next_state_dist, state, (weight_hidden_state, weight_map) = self.dynamic(
                state, self.external_input_seq_embed[i], (weight_hidden_state, ))
            state_sampled_list.append(state)
            weight_map_list.append(weight_map)

        self.sampled_state = torch.stack(state_sampled_list)
        ...
        _, posterior_lstm_state = self.rnn(
            torch.cat([self.external_input_seq_embed, self.process_x(observations_sample)], dim=-1), posterior_lstm_state
        )

        return observations_dist, observations_sample, {
    ...
    def call_loss(self):
        ...
        def kl_sample(D):
            ...
            for step in range(D):
                ...
                external_input_need = self.external_input_seq_embed[-length:]
            ...
            #                                          )
            # return kl
        kl = sum([kl_sample(D) for _ in range(self.R)])/self.R
        # endregion
