...
class MechanismODE(nn.Module, BaseModel):

    def __init__(self, h_size, y_size, u_size, theta=1.0, mu=.00, sigma=0.5, inter='gp',
                 dt=1e-2, rtol=1e-3, atol=1e-3, method='euler', adaptive=False):
        super(MechanismODE, self).__init__()
        ...
        # TODO: 参数化
        self.gradient_net = MLP(h_size, 30, h_size, num_mlp_layers=1)
    ...
    def sample_p(self, ts, us, batch_size, y0=None, eps=None, bm=None):
        ...
        return odeint(func=self.ode_func, y0=y0, t=ts[:, 0].ravel())
    ...
    def _forward_posterior(self, external_input_seq, observations_seq, memory_state=None):
        ...
        ts = dt2ts(dt)
        ys = self.sample_q(
            ts, repeat_tail(external_input_seq), repeat_tail(observations_seq), batch_size, y0=y0, eps=None, bm=None
        )
        ys, y = ys[:-1], ys[-1]
        outputs = {
            'state_mu': ys,
    ...
    def call_loss(self, external_input_seq, observations_seq, memory_state=None):
        ...
        external_input_seq, dt = external_input_seq[..., :-1], external_input_seq[..., -1:]
        ts = dt2ts(dt)
        ys, kl = self.forward(ts, repeat_tail(external_input_seq), repeat_tail(observations_seq), batch_size, eps=None)
        ys = ys[:-1]
