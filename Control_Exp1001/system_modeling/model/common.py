...
class NCDEFunc(torch.nn.Module):
    def __init__(self, y_size, u_size, batch_size):
        ...
        self.y_size, self.u_size, self.batch_size = y_size, u_size+1, batch_size
        self.linear = torch.nn.Linear(self.y_size,
                                      self.y_size * self.u_size)

    def forward(self, t, z):
        return self.linear(z).view(self.batch_size, self.y_size, self.u_size)
