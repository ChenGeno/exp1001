...
class NCDEFunc(torch.nn.Module):
    def __init__(self, y_size, u_size, batch_size):
        super(NCDEFunc, self).__init__()
        self.y_size, self.u_size, self.batch_size = y_size, u_size, batch_size

        self.linear1 = torch.nn.Linear(y_size, 128)
        self.linear2 = torch.nn.Linear(128, u_size * y_size)
        # self.mlp1 = MLP(y_size, 128*2, 128, num_mlp_layers=3)
        # self.mlp2 = MLP(128, 128*2, u_size*y_size, num_mlp_layers=3)
    ...
    def forward(self, t, z):
        ...
        z = self.linear2(z)
        ######################
        # 容易忘记的问题：最好的结果往往是通过添加最后的tanh非线性来获得。
        ######################
        z = torch.tanh(z)
        # z = z.tanh()
        ######################
        # 忽略批量维度，输出张量的形状必须是一个矩阵、
        #         # 因为我们需要它来表示从R^input_channels到R^hidden_channels的线性映射。
        ######################
        z = z.view(z.size(0), self.y_size, self.u_size)
        return z
